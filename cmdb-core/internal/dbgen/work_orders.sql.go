...
const countPendingWorkOrders = `-- name: CountPendingWorkOrders :one
...
func (q *Queries) CountPendingWorkOrders(ctx context.Context, tenantID uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countPendingWorkOrders, tenantID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
