...
func (w *WorkflowSubscriber) createLowQualityWorkOrder(ctx context.Context, tenantID, assetID uuid.UUID) error {
	...
	// Resolve asset name/tag inside the tx for a stable snapshot and so
	// we never hand the operator a stale tag if the asset was just
	// renamed mid-scan.
	var assetName, assetTag string
	if err := tx.QueryRow(ctx,
		`SELECT name, asset_tag FROM assets WHERE id = $1 AND tenant_id = $2`,
		assetID, tenantID).Scan(&assetName, &assetTag); err != nil {
	...
	order, err := w.maintenanceSvc.CreateTx(ctx, tx, tenantID, sysUID, maintenance.CreateOrderRequest{
		Title:    fmt.Sprintf("Data Quality: %s (%s) score below %.0f for %dd", assetName, assetTag, lowQualityThreshold, lowQualityLookbackDays),
		...
		Description: fmt.Sprintf(
			...
			assetName, assetTag, lowQualityThreshold, lowQualityLookbackDays),
	})
	if err != nil {
